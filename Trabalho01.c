...
void desenho_pio(double *pattern, PIO pio, uint sm)
{
    for (int i = 0; i < NUM_PIXELS; i++)
    {
        pio_sm_put_blocking(pio, sm,
                            matrix_rgb(pattern[24 - i], 0, 0));
    }
}

double *definir_desenho(void)
{
...
void gpio_irq_handler(uint gpio, uint32_t events)
{
    ...
    else if (gpio == BUTTON_B && (events & GPIO_IRQ_EDGE_FALL))
    {
        ...
        pwm_set_gpio_level(LED_PIN_RED, 0);
        printf("Leitura reiniciada e LED apagado\n");
    }
}
...
int main(void)
{
    ...
    while (true)
    {
        ...
        if (!prev_on)
        {
            pwm_set_gpio_level(LED_PIN_RED, (uint32_t)(valor_max_adc * PWM_WRAP));
            ssd1306_fill(&ssd, false);
        ...
        double *pattern = definir_desenho();
        desenho_pio(pattern, pio, sm);
        if (feedback_pending)
        {
            // quando sem porcentagem, apaga LED
            ...
            {
                pwm_set_gpio_level(LED_PIN_RED, (uint32_t)(valor_max_adc * PWM_WRAP));
                double start = BUZZER_FREQ_MIN + valor_max_adc * (BUZZER_FREQ_MAX - BUZZER_FREQ_MIN);
                for (int i = 0; i < 10; i++)
                {
